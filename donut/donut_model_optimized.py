...
class SwinEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.model.pos_drop = nn.Dropout(0.0)

        # weight init with swin
        if not name_or_path:
            cache_dir = "cache"
            cache_file = os.path.join(cache_dir, "swin_base_patch4_window12_384.pth")
            if os.path.exists(cache_file):
                swin_state_dict = torch.load(cache_file)
            else:
                swin_state_dict = timm.create_model("swin_base_patch4_window12_384", pretrained=True).state_dict()
                os.makedirs(cache_dir, exist_ok=True)
                torch.save(swin_state_dict, cache_file)
...
class BARTDecoder(nn.Module):
    ...
    def __init__(
        ...
        if not name_or_path:
            cache_dir = "cache"
            cache_file = os.path.join(cache_dir, "asian-bart-ecjk.pth")
            if os.path.exists(cache_file):
                bart_state_dict = torch.load(cache_file)
            else:
                bart_state_dict = MBartForCausalLM.from_pretrained("hyunwoongko/asian-bart-ecjk").state_dict()
                os.makedirs(cache_dir, exist_ok=True)
                torch.save(bart_state_dict, cache_file)
            ...
            for x in new_bart_state_dict:
                if x.endswith("embed_positions.weight") and self.max_position_embeddings != 1024:
                    ...
                    bart_vocab_size = bart_state_dict[x].shape[0]
                    new_bart_state_dict[x][:bart_vocab_size, :] = bart_state_dict[x]
                else:
    ...
    def prepare_inputs_for_inference(self, input_ids: torch.Tensor, encoder_outputs: torch.Tensor, past_key_values=None, past=None, use_cache: bool = None, attention_mask: torch.Tensor = None, **kwargs):
        """
        Args:
    ...
    def forward(
        ...
        if encoder_hidden_states is not None and len(encoder_hidden_states.shape) == 4:
...
class DonutModel(PreTrainedModel):
    ...
    def __init__(self, config: DonutConfig):
        ...
        self.encoder_proj = nn.Linear(1024, 1024)
        self.counter = 0
    ...
    def forward(self, image_tensors: torch.Tensor, decoder_input_ids: torch.Tensor, decoder_labels: torch.Tensor):
        ...
        for i in range(batch_size):
            self.counter += 1
            # print(f"Generated embedding for image {self.counter}")
            sys.stdout.flush()
    ...
    def inference(
        ...
        last_hidden_state = self.encoder_proj(last_hidden_state)
        last_hidden_state = last_hidden_state.to(dtype=torch.float32, device=self.device)
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # Check for local custom vocabulary path and load tokenizer from there if exists
        local_vocab_path = "/workspace/custom_donut_vocab"
        if os.path.exists(local_vocab_path):
            model.decoder.tokenizer = XLMRobertaTokenizer.from_pretrained(local_vocab_path)
            model.decoder.model.resize_token_embeddings(len(model.decoder.tokenizer))
